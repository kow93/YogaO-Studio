...
export const FinancialReport: React.FC<FinancialReportProps> = ({ memberships, expenses, students }) => {
    ...
    const {
        ...
        const tr = fm.reduce((acc, m) => acc + m.price, 0);
        const te = fe.reduce((acc, e) => acc + e.amount, 0);
        ...
        const incomeTransactions = fm.map(m => ({
            date: m.startDate,
            type: '매출',
            description: `${students.find(s => s.id === m.studentId)?.name || '알수없음'} - ${m.passType}`,
            amount: m.price
        }));
        const expenseTransactions = fe.map(e => ({
            date: e.date,
            type: '지출',
